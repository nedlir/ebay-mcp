...
describe("Inventory Tools Integration Tests", () => {
  ...
  describe("ebay_get_offers", () => {
    it("should retrieve offers successfully", async () => {
      ...
      mockEbayApiEndpoint(
        "/sell/inventory/v1/offer?sku=TEST-001",
        "get",
        "sandbox",
        mockResponse
      );
    ...
    it("should filter by marketplace", async () => {
      const mockResponse = {
        total: 1,
        offers: [
          { offerId: "123", marketplaceId: "EBAY_US" },
        ],
      };

      mockEbayApiEndpoint(
        "/sell/inventory/v1/offer?marketplaceId=EBAY_US",
        "get",
        "sandbox",
        mockResponse
      );
